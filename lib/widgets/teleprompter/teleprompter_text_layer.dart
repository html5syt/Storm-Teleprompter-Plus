...
class _VisualTextRow {
  final double top;
  final double bottom;
  final int? firstRawIndex;
  final int? lastRawIndex;

  const _VisualTextRow({
    required this.top,
...
class TeleprompterTextLayerState extends State<TeleprompterTextLayer> {
  ...
  _VisualTextRow? _visualRowAtContentOffset(
    double contentOffset,
    double textWidth,
    TextDirection textDirection,
  ) {
    ...
    final lineIndex = _findLineForContentOffset(contentOffset);
    final line = widget.lines[lineIndex];
    final lineTop = _lineTops[lineIndex];
    if (line.characters.isEmpty) {
      return _VisualTextRow(
        top: lineTop,
        bottom: lineTop + _lineHeights[lineIndex],
      );
    }

    final painter = _layoutLinePainter(line, textWidth, textDirection);
    final localY = (contentOffset - lineTop - _lineVerticalPadding)
        .clamp(0.0, painter.height)
        .toDouble();
    final metrics = painter.computeLineMetrics();
    final metric = metrics.firstWhere((candidate) {
      final top = candidate.baseline - candidate.ascent;
      final bottom = top + candidate.height;
      return localY >= top - 0.5 && localY <= bottom + 0.5;
    }, orElse: () => metrics.last);
    final metricTop = metric.baseline - metric.ascent;
    final metricBottom = metricTop + metric.height;
    final sampleY = (metricTop + metricBottom) / 2;
    final firstOffset = painter
        .getPositionForOffset(Offset(0, sampleY))
        .offset
        .clamp(0, line.characters.length - 1)
        .toInt();
    final endOffset = painter
        .getPositionForOffset(Offset(textWidth, sampleY))
        .offset
        .clamp(0, line.characters.length)
        .toInt();
    final lastOffset = (endOffset <= firstOffset ? firstOffset : endOffset - 1)
        .clamp(firstOffset, line.characters.length - 1)
        .toInt();
    painter.dispose();

    return _VisualTextRow(
      top: lineTop + _lineVerticalPadding + metricTop,
      bottom: lineTop + _lineVerticalPadding + metricBottom,
      firstRawIndex: line.characters[firstOffset].rawIndex,
      lastRawIndex: line.characters[lastOffset].rawIndex,
    );
  }

  _VisualTextRow? _visualRowForRawIndex(
    int rawIndex,
    double textWidth,
    TextDirection textDirection,
  ) {
    ...
    if (rawIndex < 0) {
      for (final line in widget.lines) {
        if (line.characters.isNotEmpty) {
          return _visualRowForRawIndex(
            line.characters.first.rawIndex,
            textWidth,
            textDirection,
          );
        }
    ...
    final lineIndex = _findLineForRawIndex(rawIndex);
    final line = widget.lines[lineIndex];
    if (line.characters.isEmpty) return null;
    final painter = _layoutLinePainter(line, textWidth, textDirection);
    final charOffset = _charOffsetInLine(lineIndex, rawIndex);
    final boxes = painter.getBoxesForSelection(
      TextSelection(baseOffset: charOffset, extentOffset: charOffset + 1),
    );
    final caret = painter.getOffsetForCaret(
      TextPosition(offset: charOffset),
      Rect.zero,
    );
    final localY = boxes.isNotEmpty
        ? (boxes.first.top + boxes.first.bottom) / 2
        : caret.dy;
    painter.dispose();
    return _visualRowAtContentOffset(
      _lineTops[lineIndex] + _lineVerticalPadding + localY,
      textWidth,
      textDirection,
    );
  }

  /// 返回当前视觉行及其上下各两行所覆盖的原稿索引范围。
  ///
  /// 仅在启动 ASR 前调用，避免识别过程中反复测量文本布局。
  ({int startRawIndex, int endRawIndex})? asrSearchWindow() {
    if (_lineTops.isEmpty && widget.lines.isEmpty) return null;
    final viewportWidth =
    ...
    if (_lineTops.isEmpty) return null;
    final currentRow = _visualRowForRawIndex(
      widget.currentIndex,
      textWidth,
      textDirection,
    );
    ...
    final rowsBefore = <_VisualTextRow>[];
    var previous = currentRow;
    for (var count = 0; count < 2 && previous.top > 0; count++) {
      final row = _visualRowAtContentOffset(
        previous.top - 0.5,
        textWidth,
        textDirection,
      );
      if (row == null || row.top >= previous.top - 0.1) break;
      rowsBefore.add(row);
      previous = row;
    }

    final rowsAfter = <_VisualTextRow>[];
    var next = currentRow;
    for (var count = 0; count < 2 && next.bottom < _contentHeight; count++) {
      final row = _visualRowAtContentOffset(
        next.bottom + 0.5,
        textWidth,
        textDirection,
      );
      if (row == null || row.bottom <= next.bottom + 0.1) break;
      rowsAfter.add(row);
      next = row;
    }

    final rows = <_VisualTextRow>[
      ...rowsBefore.reversed,
      currentRow,
      ...rowsAfter,
    ];
    final ranges = rows
        .where((row) => row.firstRawIndex != null && row.lastRawIndex != null)
        .toList();
    if (ranges.isEmpty) return null;
    final start = ranges
        .map((row) => row.firstRawIndex!)
        .reduce((value, element) => value < element ? value : element);
    final end = ranges
        .map((row) => row.lastRawIndex!)
        .reduce((value, element) => value > element ? value : element);
    return (startRawIndex: start, endRawIndex: end);
  }
