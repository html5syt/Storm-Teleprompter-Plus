// TODO(delete-after-acceptance): 遗留提词层实现，当前页面主链未引用，仅保留删除标记。
import {
  useCallback,
  useEffect,
  useMemo,
...
export const TeleprompterView: React.FC<TeleprompterViewProps> = ({
  ...
  const isLocalBrowserEngine = feishuConfig.engineType === 'local_browser';
  const speechScrollMode = isLocalBrowserEngine ? 'local' : 'cloud';
  const baseLookaheadChars = isLocalBrowserEngine ? 8 : 13;
  ...
  const scrollLookaheadChars =
    settings.scrollMode === 'local' || settings.scrollMode === 'cloud'
      ? baseLookaheadChars
      : 0;

  const activeAsrLabel =
    speechScrollMode === 'local'
      ? 'Local ASR Active'
      : 'Cloud ASR Active';
  ...
  const updateCurrentIndex = useCallback((index: number, isRemote: boolean = false) => {
    ...
    if (isLocalBrowserEngine && isRemote) {
  ...
  useEffect(() => {
    ...
    scrollFrameRef.current = requestAnimationFrame(() => {
      ...
      const lookaheadIndex = Math.min(
        Math.max(0, currentIndex) + scrollLookaheadChars,
        totalChars - 1,
      );
  ...
  }, [currentIndex, textRows, scrollLookaheadChars]);
  ...
  const clampRemoteAdvance = useCallback(
    (index: number, isFinal: boolean): number => {
      if (!isLocalBrowserEngine || index < 0) {
        return index;
      }
    ...
    [isLocalBrowserEngine, localAsrMaxFinalJump, localAsrMaxInterimJump],
  );

  useEffect(() => {
    const shouldAsrBeOn =
      isVisible &&
      (settings.scrollMode === 'cloud' || settings.scrollMode === 'local') &&
  ...
  const toggleScrolling = () => {
    setSettings((s) => {
      ...
      if (!nextIsScrolling && (s.scrollMode === 'cloud' || s.scrollMode === 'local')) {
        stopSpeechAsr(true);
      }
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute top-0 left-0 right-0 z-50 flex items-center justify-between bg-gradient-to-b from-black/90 to-transparent p-4">
        <div className="pointer-events-auto flex items-center gap-4">
          ...
          <h1 className="max-w-[180px] truncate text-xs font-medium text-neutral-400 sm:max-w-[280px]">
            {script.title}
          </h1>
        </div>
      ...
      >
        <div className="text-left pt-[80vh] pb-[80vh]">
          {textRows.map((row, rowIndex) => {
            const rowIsRead =
              row.chars.length > 0 && currentIndex > row.endIndex;
            ...
            return (
              ...
              >
                {row.chars.length === 0 ? (
                  ...
                  row.chars.map((token) => {
                    const isRead = currentIndex > token.index;
                    const isActive = currentIndex === token.index;
                    return (
                      <span
                        key={token.id}
                        id={token.id}
        ...
        {settings.showSettings && (
          <>
            ...
            <motion.div
              ...
              className="absolute top-0 right-0 bottom-0 z-[70] flex flex-col border-l border-white/5 bg-[#0a0a0a] shadow-2xl"
            >
              <div
                onMouseDown={(e) => {
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                  ...
                  <div className="flex flex-wrap justify-start gap-2">
                    {fontSizePresets.map((preset) => (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          setSettings((s) => ({ ...s, fontSize: preset }))
                        }
                        className={
                  ...
                  <div className="flex items-center justify-between gap-3 text-sm">
                    ...
                    <div className="flex flex-wrap items-center justify-end gap-2">
                      ...
                      <button
                        ...
                        aria-label="重置行距"
                        onClick={() =>
                          setSettings((s) => ({ ...s, lineHeight: 1.5 }))
                        }
                        className="inline-flex h-8 w-8 items-center justify-center rounded-full text-neutral-500 transition-colors hover:bg-[#1a1a1a] hover:text-white"
                      >
                        <RotateCcw size={14} />
                ...
                <div className="flex items-center justify-between gap-3 rounded-2xl border border-[#2e2e2e] bg-[#141414] p-4">
                  ...
                  <button
                    type="button"
                    title={settings.mirrorMode ? '关闭镜像模式' : '开启镜像模式'}
                    aria-label={settings.mirrorMode ? '关闭镜像模式' : '开启镜像模式'}
                    onClick={() =>
                      setSettings((s) => ({
                        ...s,
                  ...
                  <div className="flex flex-wrap justify-start gap-2">
                    {[0, 5, 10, 15, 20].map((preset) => (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          setSettings((s) => ({ ...s, paddingX: preset }))
                        }
                        className={
      ...
      <AnimatePresence>
        {isLocalBrowserEngine &&
          ...
          localAsrLoadState.stage !== 'ready' && (
            <motion.div
              initial={{ opacity: 0, y: 16 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 16 }}
              className="pointer-events-none fixed bottom-24 left-1/2 z-50 w-[90%] max-w-xl -translate-x-1/2 rounded-2xl border border-yellow-400/15 bg-black/85 px-5 py-4 shadow-2xl backdrop-blur-md"
            >
        ...
        {lastTranscription && settings.scrollMode === speechScrollMode && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 20 }}
            className="pointer-events-none fixed bottom-24 left-1/2 z-50 flex w-[90%] max-w-2xl -translate-x-1/2 items-center justify-center rounded-full border border-white/10 bg-black/80 px-6 py-3 shadow-2xl backdrop-blur-md"
          >
      ...
      <TooltipProvider delayDuration={200}>
        <div className="absolute bottom-8 left-1/2 z-50 flex -translate-x-1/2 items-center gap-1 rounded-full border-[0.5px] border-white/10 bg-black/40 p-2 shadow-2xl backdrop-blur-xl">
                ...
                <div className="relative">
                  ...
                  {settings.scrollMode === 'cloud' && settings.isScrolling && (
                    <motion.div
                      ...
                      className="absolute bottom-0 -right-1 w-1 rounded-full bg-yellow-400"
                    />
                  )}
                </div>
              </ToolbarBtn>
