...
export const ArticleCreateDialog: React.FC<ArticleCreateDialogProps> = ({
  open,
  pending,
  onOpenChange,
  onSubmit,
      ...
      <DialogContent showCloseButton={false} className="flex max-h-[85vh] max-w-2xl flex-col overflow-hidden rounded-3xl border-neutral-800 bg-neutral-950 text-white">
        <DialogHeader className="flex-row items-start justify-between gap-4">
          <div className="space-y-1.5">
            <DialogTitle className="text-xl">新建稿件</DialogTitle>
            <DialogDescription className="text-neutral-400">
              填写标题和正文，创建后会立即出现在列表中。
            </DialogDescription>
          </div>
          <button
            type="button"
        ...
        <DialogFooter>
          ...
          >
            {pending ? '创建中...' : optimizing ? '等待优化完成...' : '创建稿件'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
