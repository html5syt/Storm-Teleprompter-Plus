...
| --- | --- |
| 方向键 | 在当前显示的项目间移动选择 |
| `Enter` | 单选时打开文件夹或稿件；多选时打开操作菜单 |
| `Backspace` / `Alt+↑` | 位于子文件夹时返回上一级 |
| `Alt+←` / `Alt+→` | 导航历史后退 / 前进 |
| `Ctrl+A` | 选择当前文件夹中的全部直属项目 |
| `Ctrl+X` / `Ctrl+C` / `Ctrl+V` | 剪切 / 复制所选项目，并粘贴到当前文件夹 |
| `F2` | 重命名单个所选项目 |
| `Delete` | 确认后删除全部所选项目 |
| `Ctrl+Shift+N` | 新建文件夹 |
| `Esc` | 取消选择 |
...
| --- | --- |
| `Space` | 本地控制端开始 / 暂停 |
| 左 `Shift` / 右 `Shift` | 本地控制端切换自动 / 语音模式 |
| `←` / `→` | 将当前字向前 / 向后移动一个字 |
| `↑` / `↓` | 自动播放且速度大于 0 时减速 / 加速；其他状态按可视行移动当前字 |
| `Home` / `End` | 跳到稿件开头 / 结尾 |
| `Page Up` / `Page Down` | 按当前视口可见行数向上 / 向下移动 |
| `F11` / `Enter` / `Ctrl+Shift+F` | 切换全屏 |
| `Esc` | 退出提词 |

本地自动播放且速度大于 0 时，`↑` / `↓` 和鼠标滚轮用于调整速度，按住 `Ctrl` 可将调速步长放大 10 倍。速度为 0 时，普通滚轮按可视行移动，按住 `Ctrl` 后滚轮改为调速；暂停或语音模式下滚轮保持自然滚动。`Shift` 已单独用于切换自动 / 语音模式，不作为推荐的调速修饰键。

远程客户端的开始/暂停、模式切换、调速、方向键、`Home`、`End`、`Page Up` 和 `Page Down` 均由服务端锁定，但仍可切换本机全屏。客户端退出提词前会连续显示两次警告，第三次执行退出；退出客户端不会停止服务端会话。

稿件管理和提词器中的应用级快捷键会在文本输入框获得焦点时暂停响应，避免干扰搜索、速度和字号等输入操作。
