...
public class FileSelectorApiImpl implements FileSelectorApi {
  ...
  FileResponse toFileResponse(@NonNull Uri uri) {
    ...
    // The path above is already produced with a buffered stream copy. Returning another
    // in-memory copy duplicates the entire file and crashes on large model archives.
    return new FileResponse(
        uriPath, contentResolver.getType(uri), name, size, new byte[0], nativeError);
  }
}
